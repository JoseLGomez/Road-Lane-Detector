...
Homography::Homography( const Size& _origSize, const Size& _dstSize,
                        const vector<Point2f>& _origPoints, const vector<Point2f>& _dstPoints )
...
void Homography::getPoints(vector<Point2f>& _origPts, vector<Point2f>& _ipmPts)
{
    _origPts = m_origPoints;
    _ipmPts = m_dstPoints;
}
void Homography::applyHomography(const Mat& _inputImg, Mat& _dstImg, int _borderMode)
{
    // Generate IPM image from src
    remap(_inputImg, _dstImg, m_mapX, m_mapY, INTER_LINEAR, _borderMode);//, BORDER_CONSTANT, Scalar(0,0,0,0));
}
void Homography::applyHomographyInv(const Mat& _inputImg, Mat& _dstImg, int _borderMode)
{
    // Generate IPM image from src
    remap(_inputImg, _dstImg, m_mapX, m_mapY, INTER_LINEAR, _borderMode);//, BORDER_CONSTANT, Scalar(0,0,0,0));
}
Point2d Homography::applyHomography( const Point2d& _point )
{
    return applyHomography( _point, m_H );
}
Point2d Homography::applyHomographyInv( const Point2d& _point )
{
    return applyHomography( _point, m_H_inv );
}
Point2d Homography::applyHomography( const Point2d& _point, const Mat& _H )
{
...
Point3d Homography::applyHomography( const Point3d& _point )
{
    return applyHomography( _point, m_H );
}
Point3d Homography::applyHomographyInv( const Point3d& _point )
{
    return applyHomography( _point, m_H_inv );
}
Point3d Homography::applyHomography( const Point3d& _point, const cv::Mat& _H )
{
